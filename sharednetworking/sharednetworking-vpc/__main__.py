...
DEV_VPC_CIDR = data["ACCOUNT"]["DEV"]["VPC_CIDR"]
TEST_VPC_CIDR = data["ACCOUNT"]["TEST"]["VPC_CIDR"]
PROD_VPC_CIDR = data["ACCOUNT"]["PROD"]["VPC_CIDR"]
AZ = data["AZ"]

VPC_NAME = "shared-networking"
vpc = aws.ec2.Vpc("vpc", cidr_block=VPC_CIDR, tags={"Name": f"{VPC_NAME}-vpc"})
...
public_subnet_3 = aws.ec2.Subnet(
    "public-subnet-3",
    vpc_id=vpc.id,
...
def route_table_association(subnet, route_table, type):
    aws.ec2.RouteTableAssociation(
        f"route-table-association-{type}", subnet_id=subnet, route_table_id=route_table
    )


natgw_elastic_ip_1 = aws.ec2.Eip(
    "natgw-1-elastic-ip",
    vpc=True,
    tags={
...
natgw_1_route_table = aws.ec2.RouteTable(
    "natgw-1-public-route-table",
    vpc_id=vpc.id,
    routes=[
        aws.ec2.RouteTableRouteArgs(
            cidr_block="0.0.0.0/0",
...
natgw_elastic_ip_2 = aws.ec2.Eip(
    "natgw-2-elastic-ip",
    vpc=True,
    tags={
...
natgw_2_route_table = aws.ec2.RouteTable(
    "natgw-2-public-route-table",
    vpc_id=vpc.id,
    routes=[
        aws.ec2.RouteTableRouteArgs(
            cidr_block="0.0.0.0/0",
...
natgw_elastic_ip_3 = aws.ec2.Eip(
    "natgw-3-elastic-ip",
    vpc=True,
    tags={
...
natgw_3_route_table = aws.ec2.RouteTable(
    "natgw-3-public-route-table",
    vpc_id=vpc.id,
    routes=[
        aws.ec2.RouteTableRouteArgs(
            cidr_block="0.0.0.0/0",
...
nacl_public = aws.ec2.NetworkAcl(
    "nacl-public",
    vpc_id=vpc.id,
    subnet_ids=[
        public_subnet_1.id,
...
nacl_private = aws.ec2.NetworkAcl(
    "nacl-private",
    vpc_id=vpc.id,
    subnet_ids=[
        private_subnet_1.id,
...
public_route_dev = aws.ec2.Route(
    "public-route-dev",
    destination_cidr_block=DEV_VPC_CIDR,
    transit_gateway_id=tgw.id,
    route_table_id=public_route_table.id,
)
public_route_test = aws.ec2.Route(
    "public-route-test",
    destination_cidr_block=TEST_VPC_CIDR,
    transit_gateway_id=tgw.id,
    route_table_id=public_route_table.id,
)

public_route_prod = aws.ec2.Route(
    "public-route-prod",
    destination_cidr_block=PROD_VPC_CIDR,
    transit_gateway_id=tgw.id,
    route_table_id=public_route_table.id,
)

tgw_route_dev2 = aws.ec2transitgateway.Route(
    "tgw-route-dev2",
...
share_principal_association = aws.ram.PrincipalAssociation(
    "share-principal-association",
    principal=f"arn:aws:organizations::{MASTER_ACCOUNT}:organization/{ORGANIZATION_ID}",
    resource_share_arn=tgw_share_organization.arn,
)

tgw_id_ssm = aws.ssm.Parameter(
    "tgw-id-ssm", type="String", name=f"{VPC_NAME}-tgw-id", value=tgw.id
)
